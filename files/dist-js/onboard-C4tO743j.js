...
import { At as randomToken, Ct as handleReset, Ft as waitForGatewayReachable, Tt as normalizeGatewayTokenInput, _t as applyWizardMetadata, bt as ensureWorkspaceAndSessions, gt as DEFAULT_WORKSPACE, jt as resolveControlUiLinks, m_ as readConfigFileSnapshot, v_ as writeConfigFile } from "./reply-CeB3PWQv.js";
...
async function runNonInteractiveOnboardingLocal(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-BiaWW8v5.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-CkM9H1GQ.js");
	...
	if (!opts.skipHealth) {
		const { healthCommand } = await import("./health-Dj-k2uzN.js").then((n) => n.i);
