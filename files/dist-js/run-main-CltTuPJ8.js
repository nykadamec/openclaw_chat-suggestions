...
import { bg as installUnhandledRejectionHandler, j_ as loadDotEnv } from "./auth-profiles-C-CuFECu.js";
...
import { t as ensurePluginRegistryLoaded } from "./plugin-registry-DjQ-WGnX.js";
import { t as assertSupportedRuntime } from "./runtime-guard-BGuMyQeT.js";
import { t as emitCliBanner } from "./banner-DBrXIFy8.js";
import "./doctor-config-flow-DTDEiYYi.js";
import { n as ensureConfigReady } from "./config-guard-Bxm25YDZ.js";
...
const routeHealth = {
	...
	run: async (argv) => {
		...
		const { healthCommand } = await import("./health-7BMY2kAr.js").then((n) => n.i);
		await healthCommand({
			json,
			timeoutMs,
			verbose
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		const timeoutMs = getPositiveIntFlagValue(argv, "--timeout");
		if (timeoutMs === null) return false;
		const { statusCommand } = await import("./status-B-euTZrV.js").then((n) => n.t);
		await statusCommand({
			json,
			deep,
			all,
...
const routeSessions = {
	...
	run: async (argv) => {
		...
		const { sessionsCommand } = await import("./sessions-CWrddrzz.js");
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-DCKRYXxN.js");
...
const routeMemoryStatus = {
	...
	run: async (argv) => {
		...
		const { runMemoryStatus } = await import("./auth-profiles-C-CuFECu.js").then((n) => n.V);
...
const routes = [
	...
	routeMemoryStatus,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-CeNs0uQU.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			...
			const pathArg = positionals[0];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-CeNs0uQU.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-C9KevNoc.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-C9KevNoc.js");
...
async function closeCliMemoryManagers() {
	try {
		const { closeAllMemorySearchManagers } = await import("./search-manager-Cs3e6DNF.js").then((n) => n.n);
...
async function runCli(argv = process$1.argv) {
	...
	try {
		...
		const { buildProgram } = await import("./program-qmzdfIpc.js");
		...
		if (primary) {
			...
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-BnYu527o.js").then((n) => n.t);
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-PM1iglAX.js").then((n) => n.a);
		...
		if (!shouldSkipPluginCommandRegistration({
			...
			const { loadValidatedConfigForPluginRegistration } = await import("./register.subclis-PM1iglAX.js").then((n) => n.a);
