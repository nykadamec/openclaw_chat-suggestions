...
import { At as randomToken, Ct as handleReset, Ft as waitForGatewayReachable, Tt as normalizeGatewayTokenInput, _t as applyWizardMetadata, bt as ensureWorkspaceAndSessions, gt as DEFAULT_WORKSPACE, jt as resolveControlUiLinks, m_ as readConfigFileSnapshot, v_ as writeConfigFile } from "./reply-C8St2AJ2.js";
...
async function runNonInteractiveOnboardingLocal(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-DP0HdjP4.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-tVRof3Lv.js");
	...
	if (!opts.skipHealth) {
		const { healthCommand } = await import("./health-BVJxCtHZ.js").then((n) => n.i);
