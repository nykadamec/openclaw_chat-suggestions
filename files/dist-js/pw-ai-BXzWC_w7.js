...
import { Bm as withBrowserNavigationPolicy, Im as BrowserTabNotFoundError, Lm as assertBrowserNavigationAllowed, Mm as matchBrowserUrlPattern, Om as DEFAULT_TRACE_DIR, Rm as assertBrowserNavigationRedirectChainAllowed, am as parseRoleRef, bm as getChromeWebSocketUrl, cm as appendCdpPath, dm as normalizeCdpHttpBaseForJsonEndpoints, fm as withCdpSocket, im as getRoleSnapshotStats, jm as resolveStrictExistingPathsWithinRoot, km as DEFAULT_UPLOAD_DIR, lm as fetchJson, nm as buildRoleSnapshotFromAiSnapshot, om as formatAriaSnapshot, rm as buildRoleSnapshotFromAriaSnapshot, sm as normalizeCdpWsUrl, um as getHeadersWithAuth, wm as withNoProxyForCdpUrl, xm as markPwAiLoaded, zm as assertBrowserNavigationResultAllowed } from "./auth-profiles-C-CuFECu.js";
...
async function executeSingleAction(action, cdpUrl, targetId, evaluateEnabled, depth = 0) {
	...
	switch (action.kind) {
		...
		case "close":
			...
			break;
		case "batch":
			const nestedFailure = (await batchViaPlaywright({
				cdpUrl,
				targetId: effectiveTargetId,
			...
			if (nestedFailure) throw new Error(nestedFailure.error ?? "Nested batch action failed");
			break;
