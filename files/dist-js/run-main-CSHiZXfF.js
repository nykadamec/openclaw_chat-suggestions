...
import { bg as installUnhandledRejectionHandler, j_ as loadDotEnv } from "./auth-profiles-CWGpGLAg.js";
...
import { t as ensurePluginRegistryLoaded } from "./plugin-registry-OU9J-9WB.js";
import { t as assertSupportedRuntime } from "./runtime-guard-BGuMyQeT.js";
import { t as emitCliBanner } from "./banner-CT-kfenO.js";
import "./doctor-config-flow-DwPjTXCP.js";
import { n as ensureConfigReady } from "./config-guard-BJ2ZDAKG.js";
...
const routeHealth = {
	...
	run: async (argv) => {
		...
		const { healthCommand } = await import("./health-CswslSE1.js").then((n) => n.i);
		await healthCommand({
			json,
			timeoutMs,
			verbose
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		const timeoutMs = getPositiveIntFlagValue(argv, "--timeout");
		if (timeoutMs === null) return false;
		const { statusCommand } = await import("./status-Cnr_U8tV.js").then((n) => n.t);
		await statusCommand({
			json,
			deep,
			all,
...
const routeSessions = {
	...
	run: async (argv) => {
		...
		const { sessionsCommand } = await import("./sessions-Z3kgZmqj.js");
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-BU5DcZrh.js");
...
const routeMemoryStatus = {
	...
	run: async (argv) => {
		...
		const { runMemoryStatus } = await import("./auth-profiles-CWGpGLAg.js").then((n) => n.V);
...
const routes = [
	...
	routeMemoryStatus,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-XpAyRsht.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			...
			const pathArg = positionals[0];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-XpAyRsht.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-DFD3yRNz.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-DFD3yRNz.js");
...
async function closeCliMemoryManagers() {
	try {
		const { closeAllMemorySearchManagers } = await import("./search-manager-D6ysFFBZ.js").then((n) => n.n);
...
async function runCli(argv = process$1.argv) {
	...
	try {
		...
		const { buildProgram } = await import("./program-BqjEjYsv.js");
		...
		if (primary) {
			...
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-dhg8kLQc.js").then((n) => n.t);
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-DR7ghvZM.js").then((n) => n.a);
		...
		if (!shouldSkipPluginCommandRegistration({
			...
			const { loadValidatedConfigForPluginRegistration } = await import("./register.subclis-DR7ghvZM.js").then((n) => n.a);
